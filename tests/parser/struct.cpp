...
MAYFLY_ADD_TESTCASE("single member literal",
    test(UR"(struct { let i : int; };)",
        struct_literal{ { 0, 23 },
            { { declaration{ { 9, 20 },
                { lexer::token_type::identifier, UR"(i)", { 13, 14 } },
    ...
    test(UR"(struct { let i : int; let j = 1; };)",
        struct_literal{ { 0, 34 },
            { { declaration{ { 9, 20 },
                  ...
                  reaver::none } },
                { declaration{ { 22, 31 },
                    { lexer::token_type::identifier, UR"(j)", { 26, 27 } },
                    reaver::none,
...
MAYFLY_ADD_TESTCASE("empty declaration",
    test(UR"(struct foo {};)",
        declaration{ { 0, 13 },
            { lexer::token_type::identifier, UR"(foo)", { 7, 10 } },
            reaver::none,
    ...
    test(UR"(struct foo { let i : int; };)",
        declaration{ { 0, 27 },
            ...
            reaver::make_optional<expression>({ { 0, 27 },
                struct_literal{ { 0, 27 },
                    { { declaration{ { 13, 24 },
                        { lexer::token_type::identifier, UR"(i)", { 17, 18 } },
                        reaver::make_optional<expression>({ { 21, 24 },
                            postfix_expression{ { 21, 24 },
                                id_expression{ { 21, 24 }, { { lexer::token_type::identifier, UR"(int)", { 21, 24 } } } },
                                reaver::none,
                                {} } }),
                        reaver::none } } } } }) },
        &parse_struct_declaration));
            ...
            reaver::make_optional<expression>({ { 0, 38 },
                struct_literal{ { 0, 38 },
                    { { declaration{ { 13, 24 },
                          ...
                          reaver::none } },
                        { declaration{ { 26, 35 },
                            { lexer::token_type::identifier, UR"(j)", { 30, 31 } },
                            reaver::none,
                            reaver::make_optional<expression>(
